...
def train():
  ...
  with tf.Graph().as_default():
    ...
    cross_entropy_ll=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_logits, labels=y_label_ll));
    ...
    # Run computation
    model_var_list=slim.get_model_variables()[bn_var_num:];
    ...
    #with tf.train.MonitoredSession() as sess:
      ...
      train_writer = tf.summary.FileWriter('/tmp/nips17/attack_gan/learn_gan_ll_'+str(FLAGS.max_epsilon), sess.graph)
      # training
      ...
      saver2.save(sess,"my-models_iv3_r3/my-model_"+str(FLAGS.max_epsilon)+".ckpt");
