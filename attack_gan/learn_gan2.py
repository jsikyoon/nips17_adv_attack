...
def train():
  ...
  with tf.Graph().as_default():
    ...
    g_loss=-1*cross_entropy-1*cross_entropy2;
    ...
    with tf.Session() as sess:
      ...
      for i in range(FLAGS.max_epoch):
        ...
        for j in range(len(total_data) / FLAGS.batch_size):
          ...
          acc_p2,ce_p2,acc_p,ce_p,infi_p,_=sess.run([gen_acc2,cross_entropy2,gen_acc,cross_entropy,infi_norm,g_trainer],feed_dict={x_data: batch_data, y_label: batch_label});
          tr_infi+=infi_p;
          tr_ce+=ce_p;
          tr_gen_acc+=acc_p;
          tr_ce2+=ce_p2;
          tr_gen_acc2+=acc_p2;
        print(str(i+1)+" Epoch InfiNorm:"+str(tr_infi/(j+1))+",CE: "+str(tr_ce/(j+1))+",Acc: "+str(tr_gen_acc/(j+1))+",CE2: "+str(tr_ce2/(j+1))+",Acc2: "+str(tr_gen_acc2/(j+1)));
        total_idx=range(len(total_data)); np.random.shuffle(total_idx);
        total_data=total_data[total_idx];total_label=total_label[total_idx];
      saver_gen.save(sess,"my-models/my-model_"+str(FLAGS.max_epsilon)+".ckpt");
