...
def train():
  ...
  with tf.Graph().as_default():
    ...
    disc_var_list4=slim.get_model_variables()[(len(disc_var_list)+len(disc_var_list2)+len(disc_var_list3)):];
    ...
    disc_var_list5=slim.get_model_variables()[(len(disc_var_list)+len(disc_var_list2)+len(disc_var_list3)+len(disc_var_list4)):];

    # average
    #predicted_logits_avg=(predicted_logits+predicted_logits2+predicted_logits3+predicted_logits4+predicted_logits5)/5.0;
    ...
    gen_acc4=tf.reduce_mean(tf.cast(tf.equal(predicted_labels4,tf.argmax(y_label,1)),tf.float32));
    cross_entropy4=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_logits4, labels=y_label));
    gen_acc5=tf.reduce_mean(tf.cast(tf.equal(predicted_labels5,tf.argmax(y_label,1)),tf.float32));
    cross_entropy5=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_logits5, labels=y_label));
    gen_acc_avg=tf.reduce_mean(tf.cast(tf.equal(predicted_labels_avg,tf.argmax(y_label,1)),tf.float32));
    #cross_entropy_avg=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_logits_avg, labels=y_label));
    ...
    with tf.Session() as sess:
      ...
      saver4.restore(sess,FLAGS.checkpoint_path+FLAGS.checkpoint_file_name4);
      saver5.restore(sess,FLAGS.checkpoint_path+FLAGS.checkpoint_file_name5);
      # training
      for i in range(FLAGS.max_epoch):
        ...
        for j in range(len(total_data) / FLAGS.batch_size):
          ...
          acc_p_a,ce_p_a,acc_p5,ce_p5,acc_p4,ce_p4,acc_p3,ce_p3,acc_p2,ce_p2,acc_p,ce_p,infi_p,_=sess.run([gen_acc_avg,cross_entropy_avg,gen_acc5,cross_entropy5,gen_acc4,cross_entropy4,gen_acc3,cross_entropy3,gen_acc2,cross_entropy2,gen_acc,cross_entropy,infi_norm,g_trainer],feed_dict={x_data: batch_data, y_label: batch_label});
          tr_infi+=infi_p;
          tr_ce+=ce_p;
          tr_gen_acc+=acc_p;
          tr_ce2+=ce_p2;
        ...
        print(str(i+1)+" Epoch InfiNorm:"+str(tr_infi/(j+1))+",CE: "+str(tr_ce/(j+1))+",Acc: "+str(tr_gen_acc/(j+1))+",CE2: "+str(tr_ce2/(j+1))+",Acc2: "+str(tr_gen_acc2/(j+1))+",CE3: "+str(tr_ce3/(j+1))+",Acc3: "+str(tr_gen_acc3/(j+1))+",CE4: "+str(tr_ce4/(j+1))+",Acc4: "+str(tr_gen_acc4/(j+1))+",CE5: "+str(tr_ce5/(j+1))+",Acc5: "+str(tr_gen_acc5/(j+1))+",Acc_avg: "+str(tr_gen_acc_avg/(j+1))+",CE_avg: "+str(tr_ce_avg/(j+1)));
        total_idx=range(len(total_data)); np.random.shuffle(total_idx);
        total_data=total_data[total_idx];total_label=total_label[total_idx];
      saver_gen.save(sess,"my-models_iv3_rv250_rv2152_rv2101_iv4_avg2/my-model_"+str(FLAGS.max_epsilon)+".ckpt");
...
if __name__ == '__main__':
  ...
  parser.add_argument('--checkpoint_file_name4', type=str, default='resnet_v2_101.ckpt',
                      help='checkpoint file name4')
  parser.add_argument('--checkpoint_file_name5', type=str, default='inception_v4.ckpt',
                      help='checkpoint file name5')
